    ...
    public function __construct() {
        ...
        try {
            if (self::$sharedDbh === null) {
                ...
                if (!self::$sharedDbh->inTransaction()) {
                    try {
                        // Ensure customers table has username and password columns
                        $stmt = self::$sharedDbh->query("SHOW COLUMNS FROM customers LIKE 'username'");
                        if (!$stmt->fetch(PDO::FETCH_OBJ)) {
                            self::$sharedDbh->exec("ALTER TABLE customers ADD COLUMN username VARCHAR(100) UNIQUE NULL AFTER email");
                        }
                        $stmt = self::$sharedDbh->query("SHOW COLUMNS FROM customers LIKE 'password'");
                        if (!$stmt->fetch(PDO::FETCH_OBJ)) {
                            self::$sharedDbh->exec("ALTER TABLE customers ADD COLUMN password VARCHAR(255) NULL AFTER username");
                        }

                        // Ensure wholesaler_requests table exists
                        self::$sharedDbh->exec("CREATE TABLE IF NOT EXISTS wholesaler_requests (
                        ...
                        self::$sharedDbh->exec("CREATE TABLE IF NOT EXISTS ecommerce_retail_customers (
                            id INT AUTO_INCREMENT PRIMARY KEY,
                        ...
                        $stmt = self::$sharedDbh->query("SHOW COLUMNS FROM ecommerce_retail_customers LIKE 'username'");
                        if (!$stmt->fetch(PDO::FETCH_OBJ)) {
                            self::$sharedDbh->exec("ALTER TABLE ecommerce_retail_customers ADD COLUMN username VARCHAR(100) UNIQUE NULL AFTER email");
                        }

                        // Ensure company_settings has ecommerce_store_url
                        $stmt = self::$sharedDbh->query("SHOW TABLES LIKE 'company_settings'");
                        if ($stmt->fetch(PDO::FETCH_OBJ)) {
                            $stmtCol = self::$sharedDbh->query("SHOW COLUMNS FROM company_settings LIKE 'ecommerce_store_url'");
                            if (!$stmtCol->fetch(PDO::FETCH_OBJ)) {
                                self::$sharedDbh->exec("ALTER TABLE company_settings ADD COLUMN ecommerce_store_url VARCHAR(255) NULL DEFAULT ''");
                            }
                        }
