...
// --- E-COMMERCE CHECKOUT API ENDPOINT (WITH FILE UPLOAD & EMAIL SUPPORT) ---
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'public_checkout') {
    ...
    try {
        ...
        // Handle Receipt Upload for Bank Transfers AND Cheques (With Compression)
        $receipt_path = null;
        if (in_array($payment_method, ['Bank Transfer', 'Cheque'])) {
            if (!isset($_FILES['receipt']) || $_FILES['receipt']['error'] !== UPLOAD_ERR_OK) {
                throw new Exception("A payment receipt/image is required for {$payment_method}.");
            }
        ...
        // Note: order_status is added automatically via DB Migration in header.php
        $stmt = $pdo->prepare("INSERT INTO orders (customer_id, shipping_name, shipping_phone, shipping_address, subtotal, total_amount, payment_method, payment_receipt, payment_status, order_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')");
        $stmt->execute([$customer_id, $shipping_name, $shipping_phone, $shipping_address, $subtotal, $subtotal, $payment_method, $receipt_path, $payment_status]);
...
<body>
            ...
            <div class="col-lg-7 p-4 p-md-5">
                ...
                <form id="checkoutForm" onsubmit="processCheckout(event)">
                    ...
                    <div class="row g-3 mb-4">
                            ...
                            <div class="bg-primary bg-opacity-10 border border-primary border-opacity-25 rounded p-4 shadow-sm">
                                ...
                                <div class="mb-2 d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <span class="fw-bold text-dark fs-6">G L D Susara Senarathne</span>
                                    </div>
                                ...
                                <div class="mb-3 d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <span class="fw-bold text-dark fs-4">8018526390</span>
                                    </div>
                        ...
                        <!-- Cheque Option -->
                        <div class="col-12">
                            <div class="payment-method-card" id="cardCheque" onclick="selectPaymentMethod('Cheque')">
                                <input class="form-check-input me-3 mt-0 fs-5" type="radio" name="payment_method" value="Cheque" id="radioCheque">
                                <div class="flex-grow-1">
                                    <div class="fw-bold text-dark fs-6">Cheque Payment</div>
                                    <small class="text-muted">Upload an image of your cheque. Original required on delivery.</small>
                                </div>
                                <i class="bi bi-card-heading fs-2 text-warning text-dark opacity-75"></i>
                            </div>
                        </div>
                        
                        <!-- Cheque Instructions Block (Hidden by default) -->
                        <div class="col-12 d-none" id="chequeDetailsBlock">
                            <div class="bg-warning bg-opacity-10 border border-warning border-opacity-50 rounded p-4 shadow-sm">
                                <h6 class="fw-bold text-dark mb-2"><i class="bi bi-info-circle text-warning"></i> Cheque Instructions</h6>
                                <p class="small text-muted mb-0">Please write the cheque in favor of <strong>"Fintrix Distributions"</strong>. Ensure the amount matches the exact order total. You must upload a clear photo of the written cheque below to confirm the order.</p>
                            </div>
                        </div>
                        
                        <!-- Shared Receipt Upload Block (For Bank and Cheque) -->
                        <div class="col-12 d-none" id="receiptUploadWrapper">
                            <div class="bg-light p-3 border rounded shadow-sm">
                                <label class="form-label fw-bold small text-dark">Upload Payment/Cheque Proof <span class="text-danger">*</span></label>
                        ...
                        <div class="col-12">
                            <div class="payment-method-card disabled">
                                ...
                                <i class="bi bi-credit-card fs-2 text-muted opacity-50"></i>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function selectPaymentMethod(method) {
            ...
            document.getElementById('cardCheque').classList.remove('active');
            
            document.getElementById('bankDetailsBlock').classList.add('d-none');
            document.getElementById('chequeDetailsBlock').classList.add('d-none');
            document.getElementById('receiptUploadWrapper').classList.add('d-none');
            ...
            if (method === 'COD') {
                ...
                document.getElementById('receiptUploadWrapper').classList.remove('d-none');
            } else if (method === 'Cheque') {
                document.getElementById('cardCheque').classList.add('active');
                document.getElementById('radioCheque').checked = true;
                document.getElementById('chequeDetailsBlock').classList.remove('d-none');
                document.getElementById('receiptUploadWrapper').classList.remove('d-none');
            }
        }
        ...
        async function processCheckout(e) {
            ...
            if ((paymentMethod === 'Bank Transfer' || paymentMethod === 'Cheque') && receiptInput.files.length === 0) {
                alert("Please upload your proof of payment/cheque to complete the order.");
                return;
            }
