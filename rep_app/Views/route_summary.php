...
<div class="summary-card">
    ...
    <!-- Pending GL Section (NEW) -->
    <?php if(!empty($s['pending_gl'])): ?>
    <div style="margin-bottom: 25px; background: rgba(251, 191, 36, 0.05); border: 1px solid #fbbf24; border-radius: 8px; padding: 15px;">
        <div class="section-title" style="color: #b45309;">📄 Pending GL (Awaiting Verification)</div>
        <p style="margin-top: 0; margin-bottom: 10px; font-size: 13px; color: #78350f;">
            <?= count($s['pending_gl']) ?> collection(s) recorded and pending finalization in the GL.
        </p>
        
        <?php foreach($s['pending_gl'] as $entry): ?>
        <div style="background: white; padding: 10px; border-radius: 4px; margin-bottom: 8px; border-left: 3px solid #fbbf24;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                <span style="font-size: 13px; font-weight: bold; color: #333;">
                    <?= htmlspecialchars($entry->customer_name ?? 'Unknown Customer') ?>
                </span>
                <span style="font-size: 12px; background: #fef3c7; color: #92400e; padding: 2px 8px; border-radius: 3px; font-weight: bold;">
                    <?= htmlspecialchars($entry->payment_method) ?>
                </span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; font-size: 13px;">
                <span style="color: #666;">
                    Amount: <strong>Rs: <?= number_format($entry->amount, 2) ?></strong>
                </span>
            ...
            <?php if($entry->payment_method === 'Cheque' && !empty($entry->cheque_number)): ?>
            <div style="font-size: 12px; color: #666; margin-top: 4px;">
                Cheque #: <?= htmlspecialchars($entry->cheque_number) ?> 
                <?php if(!empty($entry->cheque_date)): ?>
                | Clearing: <?= htmlspecialchars($entry->cheque_date) ?>
                <?php endif; ?>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div style="text-align: center; margin-top: 12px;">
            <a href="<?= APP_URL ?>/accounting/pending_collections" style="font-size: 12px; color: #0066cc; text-decoration: none; font-weight: bold;">
                Go to GL Finalization →
            </a>
        </div>
    </div>
    <?php endif; ?>
