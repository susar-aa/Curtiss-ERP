...
$totalOutstanding = 0;
if (!empty($data['estimate']->customer_id)) {
    ...
    ");
    $db->bind(':id', $data['estimate']->customer_id);
...
if (empty($repName) && !empty($data['estimate']->rep_route_id)) {
    $db->query("
        ...
        LIMIT 1
    ");
    $db->bind(':route_id', $data['estimate']->rep_route_id);
    $repRow = $db->single();
    if ($repRow) {
...
if (empty($repName) && !empty($data['estimate']->user_id)) {
    $db->query("
        ...
        LIMIT 1
    ");
    $db->bind(':user_id', $data['estimate']->user_id);
    $repRow = $db->single();
    if ($repRow) {
        $repName = trim($repRow->rep_name);
        $repPhone = $repRow->rep_phone ?? '';
...
$paymentTermName = '';
if (!empty($data['estimate']->payment_term_id)) {
    $db->query("SELECT name FROM payment_terms WHERE id = :id LIMIT 1");
    $db->bind(':id', $data['estimate']->payment_term_id);
    $termRow = $db->single();
    if ($termRow) {
...
// Calculations
$subTotal = floatval($data['estimate']->total_amount ?? 0);
$globalDiscountAmount = 0;
$globalDiscVal = floatval($data['estimate']->global_discount_val ?? 0);

if($globalDiscVal > 0) {
    if (!empty($data['estimate']->global_discount_type) && $data['estimate']->global_discount_type == '%') {
...
$taxAmount = floatval($data['estimate']->tax_amount ?? 0);
$thisInvoiceGrandTotal = $netSubTotal + $taxAmount;

$estimatePaidAmount = floatval($data['invoice_paid'] ?? 0);
$statusStr = strtolower(trim($data['estimate']->status ?? ''));
...
<body>
    ...
    <div class="controls-container">
        <div class="controls-title">
            <span>Official Estimate View</span>
        ...
        <div class="print-controls">
            ...
            <?php if(!empty($data['estimate']->id) && is_numeric($data['estimate']->id)): ?>
                <a href="<?= APP_URL ?>/sales/download_pdf/<?= $data['estimate']->id ?>" class="btn-action btn-pdf"><i class="ph ph-file-pdf"></i> PDF</a>
            <?php endif; ?>
    ...
    <div class="page-wrapper">
        ...
        <div class="main-content">
            ...
            <div class="invoice-header">
                ...
                <div class="invoice-meta">
                    ...
                    <table class="meta-table">
                        <tr>
                            <th>Estimate No:</th>
                            <td><?= htmlspecialchars($data['estimate']->estimate_number) ?></td>
                        </tr>
                        <tr>
                            <th>Date:</th>
                            <td><?= date('d-M-Y', strtotime($data['estimate']->estimate_date ?? date('Y-m-d'))) ?></td>
                        </tr>
                        ...
                        <?php endif; ?>
                        <?php if(!empty($data['estimate']->payment_method)): ?>
                        <tr>
                            <th>Payment:</th>
                            <td><?= htmlspecialchars($data['estimate']->payment_method) ?></td>
                        </tr>
                        <?php endif; ?>
                        <?php if(!empty($data['estimate']->cheque_date)): ?>
                        <tr>
                            <th>Cheque Date:</th>
                            <td><?= date('d-M-Y', strtotime($data['estimate']->cheque_date)) ?></td>
                        </tr>
                        <?php endif; ?>
                    </table>
                </div>
            ...
            <div class="customer-section">
                <div class="info-card">
                    <div class="customer-name"><?= htmlspecialchars($data['estimate']->customer_name ?? 'Customer') ?></div>
                    <div class="customer-details">
                        <?php if(!empty($data['estimate']->address)) echo nl2br(htmlspecialchars($data['estimate']->address)) . '<br>'; ?>
                        <?php if(!empty($data['estimate']->phone)) echo '<strong>Tel:</strong> ' . htmlspecialchars($data['estimate']->phone); ?>
                    </div>
                </div>
                ...
                <div class="info-card">
                    ...
                    <?php else: ?>
                        ...
                        <div class="customer-details" style="margin-top: 4px;">
                            ...
                            <?php endif; ?>
                            <?php if(!empty($data['estimate']->payment_method)): ?>
                                <div style="margin-top: 6px;"><strong>Method:</strong> <?= htmlspecialchars($data['estimate']->payment_method) ?></div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="summary-card">
                    <table class="table-totals">
                        <?php if($globalDiscVal > 0): ?>
                            ...
                            <tr>
                                <th>Discount (<?= (!empty($data['estimate']->global_discount_type) && $data['estimate']->global_discount_type == '%') ? number_format($globalDiscVal, 2) . '%' : 'Flat' ?>):</th>
        ...
        <div class="document-footer">
            ...
            <div>Official Digital Estimate</div>
        </div>
    </div>
    ...
    <script>
        function exportToExcel() {
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td colspan="2" style="text-align: right; border-bottom: 2px solid #000; padding-bottom: 10px;">
                                <strong>Estimate No:</strong> <?= htmlspecialchars($data['estimate']->estimate_number) ?><br>
                                <strong>Date:</strong> <?= date('d-M-Y', strtotime($data['estimate']->estimate_date ?? date('Y-m-d'))) ?><br>
                                <?php if(!empty($paymentTermName)): ?>
                                <strong>Payment Term:</strong> <?= htmlspecialchars($paymentTermName) ?><br>
                                <?php endif; ?>
                                <?php if(!empty($data['estimate']->payment_method)): ?>
                                <strong>Payment:</strong> <?= htmlspecialchars($data['estimate']->payment_method) ?><br>
                                <?php endif; ?>
                                <?php if(!empty($data['estimate']->cheque_date)): ?>
                                <strong>Cheque Date:</strong> <?= date('d-M-Y', strtotime($data['estimate']->cheque_date)) ?><br>
                                <?php endif; ?>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="3" style="padding-top: 5px; vertical-align: top;">
                                <strong style="font-size: 11pt;"><?= htmlspecialchars($data['estimate']->customer_name ?? 'Customer') ?></strong><br>
                                <?php if(!empty($data['estimate']->address)) echo nl2br(htmlspecialchars($data['estimate']->address)) . '<br>'; ?>
                                <?php if(!empty($data['estimate']->phone)) echo 'Tel: ' . htmlspecialchars($data['estimate']->phone); ?>
                            </td>
                        ...
                        <?php if($globalDiscVal > 0): ?>
                        <tr>
                            <th style="text-align: right; padding: 4px;">Discount (<?= (!empty($data['estimate']->global_discount_type) && $data['estimate']->global_discount_type == '%') ? number_format($globalDiscVal, 2) . '%' : 'Flat' ?>):</th>
                        ...
                        <?php if($taxAmount > 0): ?>
                        <tr>
                            <th style="text-align: right; padding: 4px;">Tax (<?= htmlspecialchars($data['estimate']->tax_name ?? 'Tax') ?> <?= $data['estimate']->rate_percentage ?? '' ?>%):</th>
            ...
            link.download = 'Estimate_<?= htmlspecialchars($data['estimate']->estimate_number) ?>.xls';
