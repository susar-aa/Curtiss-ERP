...
<body>
    <div class="no-print no-print-filter">
        <form method="GET">
            ...
            <button type="submit" style="padding: 6px 12px; background: #f59e0b; color:#fff; border:none; border-radius:4px; font-weight:600; cursor:pointer;">
                Filter
            </button>
            <?php if ($data['start_date'] || $data['end_date']): ?>
                <a href="<?= APP_URL ?>/payment/statement/supplier/<?= $data['supplier']->id ?>" style="color: #ef6c00; text-decoration: none; font-size: 13px; font-weight: 600; margin-left: 10px;">Clear</a>
            <?php endif; ?>
        </form>
    ...
    <div class="info-grid">
        <div class="info-block">
            ...
            <p style="font-size: 16px; font-weight: 700; color: #111; margin-bottom: 6px;"><?= htmlspecialchars($data['supplier']->name) ?></p>
            <p><?= htmlspecialchars($data['supplier']->address ?: 'No Address Stated') ?></p>
            <p>Phone: <?= htmlspecialchars($data['supplier']->phone ?: 'N/A') ?></p>
            <p>Email: <?= htmlspecialchars($data['supplier']->email ?: 'N/A') ?></p>
        </div>
        <div class="info-block" style="text-align: right;">
            <h3>Financial Summary</h3>
            <p>Total Purchase Billed: <strong>Rs <?= number_format($data['stats']->total_invoiced, 2) ?></strong></p>
            <p>Total Payments Made: <strong>Rs <?= number_format($data['stats']->total_paid, 2) ?></strong></p>
            <p>Outstanding Liability: <strong style="color: #ef4444;">Rs <?= number_format($data['stats']->outstanding, 2) ?></strong></p>
        </div>
    </div>

    <div class="stats-summary">
        <div class="stat-item">
            <div class="stat-label">Total Purchased (Credits)</div>
            <div class="stat-val" style="color: #b45309;">Rs <?= number_format($data['stats']->total_invoiced, 2) ?></div>
        </div>
        <div class="stat-item">
            <div class="stat-label">Total Cash Paid (Debits)</div>
    ...
    <table class="ledger-table">
        <thead>
            <tr>
                ...
                <th style="text-align: right; width: 130px;">Debit (-) (Payments)</th>
                <th style="text-align: right; width: 130px;">Credit (+) (Bills)</th>
